...
const About = () => {
    ...
    const paragraphDivsRef = useRef<NodeListOf<Element>>(document.querySelectorAll("#paragraph-div"));
    useEffect(() => {
    ...
    return (
        <div name="about" className={`w-full flex flex-col items-center justify-center pb-10 transition-all duration-500 md:h-screen`}>
                ...
                <div className={`col-span-3 flex flex-col gap-3 px-5 md:px-0 items-center md:items-start`}>
                    ...
                    <div className='paragraph-start-position transition-300' id='paragraph-div'>
                        <div className='flex flex-col items-center justify-between md:text-start md:items-start md:justify-start gap-0.5'>
                            <Markdown className="prose prose-lg prose-neutral dark:prose-invert duration-200 prose-strong:text-red-600">{description}</Markdown>
                        </div>
                        <div className='flex justify-center md:justify-start'>
                            <button className={`bg-red-600 text-white text-lg px-3 py-4 transition-all mt-5
                                duration-300 rounded-sm flex items-center gap-2 font-RobotoCondensed hover:scale-110`} onClick={() => openCurriculum()}>Download my curriculum!<IoMdPaper className='text-2xl' /></button>
                        </div>
                    </div>
                </div>
            </div>
