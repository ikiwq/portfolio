...
const MinorProject = ({
  ...
  return (
    <div
      className="flex relative rounded-sm hover:scale-110 duration-500 cursor-pointer text-white text-sm start"
      ...
      <div className="flex flex-col z-20 justify-between w-full h-full bg-red-600 px-4 py-4 bg-opacity-90 hover:bg-opacity-75 duration-300">
        <div>
          ...
          <div className="flex flex-col justify-start items-start gap-4 flex-grow">
            <p
              className="font-RobotoCondensed font-light text-lg lg:text-base"
        ...
        >
          <div className="font-RobotoCondensed font-light flex gap-2.5 flex-wrap">
            {project.technologies &&
              project.technologies.map((technology, index) => {
                return (
                  <span key={"minorProjectTechnology" + index}>
                    {technology}
                  </span>
                );
              })}
          ...
          <div
            className={`flex text-4xl items-center ${
              right ? "justify-star" : "justify-end"
            }`}
          >
            {project.linkUrl ? (
              <div onClick={(event) => openProjectUrl(event, project.linkUrl)}>
                <AiOutlineLink className="hover:cursor-pointer duration-200 hover:text-black" />
              </div>
            ) : null}
            {project.githubUrl ? (
              <div
                onClick={(event) => openProjectUrl(event, project.githubUrl)}
              >
                <AiFillGithub className="hover:cursor-pointer duration-200 hover:text-black" />
              </div>
            ) : null}
          </div>
        </footer>
