...
const NavBar = ({
  ...
  window.onscroll = function (event: Event) {
    ...
    const elements = document.querySelectorAll("#navbar-link-top");
  ...
  useEffect(() => {
    const links = document.getElementsByClassName("navbar-link-menu");
  ...
  // On load effects
  useEffect(() => {
    const timeoutId = showNavbarElements(1750);
    return () => clearTimeout(timeoutId);
  }, []);

  const showNavbarElements = (delay: number) => {
    const timeoutId = setTimeout(() => {
      const hiddenElements = document.querySelectorAll("#navbar-link-top");
  ...
  const hideNavbarElements = (delay: number) => {
    const timeoutId = setTimeout(() => {
      const shownElements = document.querySelectorAll("#navbar-link-top");
  ...
  return (
    <div
      className={`py-1 text-3xl font-Bebas fixed top-0 left-0 w-full z-50 bg-white dark:bg-black duration-300`}
      id="navbar"
    >
      {menuOpen && (
        <div
          className={`dark:bg-black bg-white h-screen w-screen absolute z-50`}
        >
          <div className="text-red-600 gap-4 justify-center items-center flex flex-col h-full text-6xl">
            ...
            <ul className="flex gap-4 pt-6">
              {socials.map((social, index) => (
                <li
                  key={`social-${index}`}
                  className={`navbar-link-menu text-secondary`}
                  onClick={() => toggleMenu()}
                >
      ...
      <div className="flex justify-between container max-w-5xl mx-auto px-2">
        <div
          className="text-red-600 navbar-link-top-no-hover"
          id="written-logo"
        >
          <a href="https://ikiwq.github.io/blog" rel="noreferr" target="_blank">
            <p
              className="font-handwrite text-4xl top-navbar-element-hidden cursor-pointer"
              id="navbar-link-top"
            >
              Iki
            </p>
          ...
          {sections.map((section, index) => (
            <li key={"section-top" + index.toString()}>
              <Link to={section.to} smooth duration={500}>
                <p
                  id="navbar-link-top"
                  className="cursor-pointer duration-200 top-navbar-element-hidden"
                >
                  {section.name}
                </p>
              </Link>
        ...
        <div
          className="flex text-red-600 navbar-link-top-no-hover items-center justify-center gap-2 text-xl z-50"
          id="darkmode-switch"
        >
          <button
            id="navbar-link-top"
            className="hover:cursor-pointer top-navbar-element-hidden duration-200"
            onClick={() => {
              toggleDarkTheme();
            }}
          >
          ...
          <button
            id="navbar-link-top"
            className="text-2xl hover:cursor-pointer md:hidden duration-200 top-navbar-element-hidden"
            onClick={() => {
              toggleMenu();
            }}
          >
